...
class ColorPointTest {
    ...
    void test3(){
        ...
        //true를 출력해야하지만 Point의 equals가 getClass를 사용해 작성되었다면 그렇지않다.
        System.out.println(onUnitCircle(p2));

        //위에가 리스코프치환법칙이 위배되는이유
        //같은 클래스가 왓을땐 true고 다른하위클래스가왓을땐 false가 나온다.
        //어쨋든 같은타입인데 하위타입이 상위타입을 대체하지못한다. 리스코프치환법칙에 위배된다.
        //시멘틱이 깨졋다. 의미가 유지되지않는다.

        //어쨋든 상속을 하게되면 equals 문제가 많이 복잡해진다.
        //counterpoint는 필드를 추가안했다.
        //그러나 colorpoint는 필드를 추가해버렸다.
        //이것들이 문제다.
        //필드를 추가했을때는 안전하고 equals의 규칙을 모두 준수하는 것은 없다.
        //자바에서도 이미 코드가 깨져있다.
    }
}