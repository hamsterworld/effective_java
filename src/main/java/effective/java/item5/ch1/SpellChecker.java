 ...
 *                                    (이것만 봐도 instance로 넘겨주고 하기때메 static은 적합x)
 *                                    --> 다시말하면 자원에따라 동작이 달라지는 클래스로는
 *                                    정적유틸리티클래스(instance생성을 막는 놈들)나 싱글톤방식으로 만들면안되는것인가?
...
public class SpellChecker {
    ...
    public static boolean isValid(String word){
        //Spellchecker만에 특별한 코드가있다고 가정
        //만약에 달랑 이렇게있으면 그냥 dictionary의 wrapper class밖에 안된다.
        //그리고 Spellchecker만에 코드들을 확인해보고싶다. 즉, 테스트해보고싶다.
        //근데 그렇게 못한다 dictionary를 바꿔넣을수없기때문이다.
        //그리고 dictionary를 사용하는데 비용도 굉장히 많이든다면?
        //Test할때마다 굉장히 비효율적이다.
        //그래서 dictionary를 mocking하고싶다.

        //또 싱글톤으로 생성하면 유연성과 재사용성이떨어진다.
        //만약에 dictionary가 영어사전인데
        //한국어사전으로 support하고싶으면 이class는 그냥못씀
        //KoreanSpellcheck해서 new koreandictionary해서 새로만들거냐?
        //유연하지못하고 재사용성도 떨어지게된다.

        //그래서 의존객체주입을해주자 --> 우리가 흔히아는 @Autowired이런거.
        //그러면 재사용도 가능해진다.
        //대신 dictionary가 interface이여야한다.
