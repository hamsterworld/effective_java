...
/**
 * ChannelTest에서도 볼수있듯이 NPE를 잡아내기는 if문으로도 간단히 가능하다.
 * 그래서 우리는 How? NPE를 덜 만날수있을까가 이번에 핵심이다.
 * Java8 부터지원하는 Optional기능을 사용한다.
 * Optional은 return type으로 쓰라고 만든거지 다른곳에서 쓰라고만든게아니다.
 */
...
//    }
     ...
     * 왜냐면 결국에는 if로 null인지아닌지 검사해야하므로 의미가없다.
     * 그리고 파라미터값이 null을 받을수있기때문이다.
     */
...
//    }
    ...
    /**
     * Optional을 collection으로 감싸지말것
     * 가령 Optional<List>이런거
     */

    /**
     * 그리고 위에처럼 return type이 Optional인데 null을 넘기지마라.
