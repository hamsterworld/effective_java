 ...
 * ex)stack, cache, listener or callback --> 일단 stack, cache, listener가 대표적인 메모리누수의 예시들
 * 위의 예시들은 공통적으로 객체를 쌓아두는 공간이 있다는 것이다.
 * 그래서 이객체를 어떻게 정리할거냐? 이정리하는 방법들도 3가지이다.
 *
 * 첫번째, 여기 Stack 예시에서처럼 null을 명시해주는것이다.
 *
 * 두번째, cache는 Stack의 pop마냥 빼는 기능이없다면?
 * 물론 cache도 데이터가 변경되면 없애야되긴한다. 일단 이번예제코드는 쌓이기만한다고생각해보자.
 * 그래서 여기서 쓰는게 weakHashMap이다.
 * 레퍼런스에는 strong ,soft ,weak, phantom 이렇게 4가지가 있다.
 * 그중에 weak 레퍼런스를 사용할거고 weak 레퍼런스를 key로 사용하는놈을 weak HashMap이라고 한다.
 * 이놈의 특징은 key가 더이상참조가안되면 weak 레퍼런스빼고 더이상참조가 안되면
 * 그 key를 가지고있는 value를 Map에서 빼준다.
 *
...
//        stack = null;
     ...
     * ㅁ-ㅁ-ㅁ-ㅁ-ㅁ-ㅁ
     * 이렇게 있다고 쳐보자.
     * 각 노드마다 ref 가 있다고 생각해보자.
     * size 를 줄여서 더는 접근못하게했다해도
     * ref 는 살아 있기때문에 날라가지않는다.
     * 만약에 계속 push 된다면 메모리가 꽉차버릴지도 모른다.
     *
     * 물론 이 main method 가 끝나면 날라가버리겟지만!
     * 멀티스레드 상황에서 생각해보면 heap 이 금방 차버릴수도있다 제때제때 비워주지않는다면?
     * 아마 금방 차버리겟지
     * 그러나 pop 하고 더이상 안쓴다고 가정하고 null 로 명시적으로 비워준다면 heap 을 아껴쓸수있다.
     * 또는 바운더리 밖으로 얼른 빼버리거나 어떻게든..
     */
}
