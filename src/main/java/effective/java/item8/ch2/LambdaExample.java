...
public class LambdaExample {
    ...
    // runnable을 이런식으로 정의해서 value같은 필드를 람다가 참조하게되면
    // 바깥객체의 참조를 가지기 쉽다. 즉,lambda라는 outer의 참조를 가지게된다.
    // 우리가 원치않은 형태가 된다.
    // 즉,이런경우 우리가 정리를 원하는 outer객체가 람다랑 서로 순환참조를 하게되기때문이다.
    // 그래서 clean는 이렇게하면안된다.
    private Runnable instanceLambda = () ->{
        System.out.println(value); // --> 물론 바깥객체의 필드든매서드든 안쓰면 안생긴다. 주석처리하고 확인해보자.
                                   //그러나 static이면 참조하지않는다 바깥객체나 람다가 static이면 아래와같은상황
    };
